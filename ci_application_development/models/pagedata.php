...
class Pagedata extends CI_Model
{
    ...
    protected function get_data_xref()
    {
        ...
        $this->db
            ...
            ->where('page_id', $this->data['page']['id']);
        $query     = $this->db->get();
    ...
    protected function get_page($page)
    {
        ...
        if (empty ($this->data['page'])) {
            ...
            ) { // Check to see if cache is configured and disable if the page isn't cacheable
                ...
                if (!is_readable($local_file)) { // Build the file
                    ...
                    if ($type == 'css') {
                        $js_id = $this->db
                            ->select('id, package')
                            ->from('CORE_JS')
                            ->where('library_name', $item['library_name'])
                            ->get()
                            ->result_array();
                        if (!empty($js_id[0]['id'])) {
                            if (!empty($js_id[0]['package'])) {
                                ...
                                foreach ($this->db
                                             ->select('item_js_id')
                                             ->from('CORE_JS_Package_XREF')
                                             ->where('package_js_id', $js_id[0]['id'])
                                             ->get()
                                             ->result_array() as $row) {
                        ...
                        $this->db
                            ->select('item_css_id')
                            ->from('CORE_CSS_Package_XREF')
                            ->where('package_css_id', $item['id'])
                            ->order_by('order', 'asc');
                        foreach ($this->db
                                     ->get()
                                     ->result_array() as $row) {
                    ...
                    foreach ($type_ids as $id) {
                        ...
                        $show_file = 'static/' . $type_dir . '/' . $item_file[0]['library_name'] . '/' . $item_file[0]['version_major'] . '/' . $item_file[0]['version_minor'] . '/' . $item_file[0]['version_build'] . '/' . $item_file[0]['file_name'];
                        $read_file = FCPATH . $show_file;
                            ...
                            if ($type == 'css') {
                                ...
                                if (!preg_match('@[-\\.](?:min|close)\\.(?:js|css)$@i', $read_file)) {
                                    ...
                                    if (is_readable($compile_file_name)) {
                                        ...
                                        $compilerCommand = sprintf('/usr/bin/java -jar %slibraries/compiler.jar --js %s --js_output_file %s', APPPATH, $read_file, $compile_file_name);
                    ...
                    $result      = $this->db
                        ->select('item_' . $type . '_id')
                        ->from('CORE_' . $BIG_type . '_Package_XREF')
                        ->where('package_' . $type . '_id', $item['id'])
                        ->get()
                        ->result_array();
                    if (!empty($result)) {
...
//				$this->data['elements'] = $elements->result_array();
    ...
    protected function cache_expiration_check()
    {
        ...
        if (!empty ($data)) {
            ...
            if ($this->rebuild_cache !== TRUE && empty($this->data['page']['split_active'])) {
