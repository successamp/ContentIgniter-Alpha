...
class News extends CI_Controller {
    ...
    function _display_latest($items=10, $post=0){
        ...
        $this->db->select('*')->from('CORE_News');
		if(ENVIRONMENT === 'production'){
			$this->db->where('active',1);
		}
		$subs = array('', $this->Pagedata->get_subdomain());
		$this->db->where_in('subdomain',$subs);
		$news_posts = $this->db->order_by('pinned','desc')->order_by('published','desc')->limit($items, $start_item)->get()->result_array();
        if(empty($news_posts)){
            show_404();
        }
