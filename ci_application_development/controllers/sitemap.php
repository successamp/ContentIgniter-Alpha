...
class Sitemap extends CI_Controller {
    ...
    function xml() {
        ...
        if(empty($latest_sitemap)){
            ...
            //### Pages
            $this->db->select('*')->from('CORE_Pages')->where('active', 1)->where('sitemap_item', 1)->where('subdomain', $subdomain)->or_where('subdomain', '')->where('sitemap_item', 1)->where('active', 1)->order_by('priority', 'DESC')->order_by('id');
            $sitemapdata_pages = $this->db->get()->result_array();
            //### News (blog)
            $this->db->select('*')->from('CORE_News')->where('active', 1)->where('sitemap_item', 1)->where('subdomain', $subdomain)->or_where('subdomain', '')->where('sitemap_item', 1)->where('active', 1)->order_by('priority', 'DESC')->order_by('published', 'DESC');
            $sitemapdata_news = $this->db->get()->result_array();
            foreach($sitemapdata_news as &$item){
                $item['url'] = 'news/'.$item['url'];
            }
            //### Merge Pages and News
            $sitemapdata = array_merge($sitemapdata_pages, $sitemapdata_news);
    		...
    		$data['template'] = array('_sitemap_header', '_blank_nav', 'sitemap_xml', '_blank_aside', '_sitemap_footer');
