...
class MY_Session extends CI_Session
{
    ...
    function login($username, $password)
    {
        if ($this->CI->config->item('sess_encrypt_cookie') == TRUE AND $this->CI->config->item('sess_use_database') == TRUE AND $this->CI->config->item('sess_table_name') != ''
            ...
            $query           = $this->CI->db
                ...
                ->from('CORE_Auth_USERS')
                ->where('username', $username)
                ->where('password', $secure_password)
                ->get();
            if ($query->num_rows() == 1) {
                foreach ($query->result_array() as $row) {
                    ...
                    $permissions           = $this->CI->db
                        ...
                        ->join('CORE_Auth_XREF', 'CORE_Auth_GROUPS.id = CORE_Auth_XREF.group_id')
                        ->where('CORE_Auth_XREF.user_id', $row['id'])
                        ->get();
    ...
    function get_user_data()
    {
        if ($this->CI->config->item('sess_encrypt_cookie') == TRUE AND $this->CI->config->item('sess_use_database') == TRUE AND $this->CI->config->item('sess_table_name') != ''
            ...
            if ($this->userdata("logged_in")) {
                $user_tables = $this->CI->db
                    ->select('table_name, handle, row')
                    ->from('CORE_Auth_DATA_XREF')
