...
class Mu extends MUQuery
{
    ...
    public function autoform($data_table, $data_array)
    {
        $query = $this->CI->db
            ->select('*')
            ->from('CORE_Table_Structure')
            ->where('table_name', $data_table)
            ->order_by('order')
            ->get()
            ->result_array();
        foreach ($query as $field) {
            $field_data = $this->CI->db
                ->select('field_PHP')
                ->from('CORE_Input_Fields')
                ->where('id', $field['input_id'])
                ->get()
                ->row_array();
    ...
    {
        $drop_down_fields  = $this->CI->db
            ->select('*')
            ->from('CORE_Drop_Down_DATA_XREF')
            ->where('table', $table)
            ->where('field_name', $field_name)
            ->get()
            ->row_array();
        $drop_down_default = $this->CI->db
            ->select('value')
            ->from('CORE_Pages_CONFIG')
            ->where('key', 'default' . $field_name)
            ->get()
            ->row_array();
        $drop_down_query   = $this->CI->db
            ->select($drop_down_fields['lookup_value_field'] . ' as VALUE_FIELD, ' . $drop_down_fields['lookup_display_field'] . ' as DISPLAY_FIELD', FALSE)
            ->from($drop_down_fields['lookup_table']);
