...
function contentigniter_cache_override($params)
{
    ...
    $user_first_entry = empty($_COOKIE['CI_user_first_entry']) ? '' : $_COOKIE['CI_user_first_entry'];
    $user_tracking    = empty($_COOKIE['CI_user_tracking']) ? '' : $_COOKIE['CI_user_tracking'];

    if (empty($user_first_entry)) {
        // Save referrer or first entry page for next 10 years
        $user_first_entry = empty($_SERVER['HTTP_REFERER']) ? $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] : $_SERVER['HTTP_REFERER'];
        setcookie('CI_user_first_entry', $user_first_entry, time() + 315360000, $CFG->item('cookie_path'), $CFG->item('cookie_domain'));
    }
    if (empty($user_tracking)) {
        ...
        setcookie('CI_user_tracking', json_encode($userdata), time() + $CFG->item('sess_expiration'), $CFG->item('cookie_path'), $CFG->item('cookie_domain'));
    }
