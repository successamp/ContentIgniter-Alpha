...
class Admin extends CI_Controller
{
    ...
    function pages_edit($page)
    {
        if (!empty ($page)) {
            ...
            $HTML  = $this->mu;
            echo $HTML
                ->form('/admin/pages_edit_update/' . $page . '/')
        ...
        if (!empty ($page)) {
            ...
            if (!empty ($page_data)) {
                foreach ($page_data as $data_row) {
                    ...
                    $page_content .= $HTML
                        ->form('/admin/pages_content_update/' . $page . '/' . $data_row['data_table_name'] . '/' . $data_row['data_id'] . '/')
                        ->addClass('simpleForm')
                        ->attr('style', 'width:600px;')
                ...
                $page_content .= $HTML
                    ->form('/admin/pages_add_content_to_page/' . $page . '/')
                    ->addClass('simpleForm')
                    ->attr('style', 'width:600px;')
                    ->label('submit', ' ')
    ...
    protected function users_change_password($username, $password)
    {
        if (!empty($username)) {
            if (strlen($password) < 9) {
                return array(
                    'class' => 'error',
                    'text'  => 'Password less than nine characters long'
                );
            }
            $query = $this->db
                ->select('*')
            ...
            if (count($query) == 1) {
                if (!empty($query[0]['create_date'])) {
                    ...
                    return array(
                        'class' => 'error',
                        'text'  => 'Unable to fetch user'
                    );
                }
            } else {
                return array(
                    'class' => 'error',
                    'text'  => 'Username was not found or was found multiple times'
                );
            }
            $salt      = $this->config->item('salt');
            $password  = hash_hmac('sha512', $password . $nonce, $salt);
            ...
            $success   = $this->db
                ->where('id', $user_id)
                ->update('CORE_Auth_USERS', $user_data);
            if ($success == 1) {
                return array(
                    'class' => 'success',
                ...
                return array(
                    'class' => 'error',
                    'text'  => 'Account was not successfully updated - Verify username and password'
                );
            }
        } else {
            return array(
