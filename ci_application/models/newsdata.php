...
class Newsdata extends Pagedata {
    ...
    protected function get_page($page) {
        ...
        if (empty ($this->data['page'])) {
            $this->db->select('*')->from('CORE_News')->where('url', $page);
        ...
        if (empty ($this->data['page']['subdomain']) && empty($this->page_config['no_subdomain'])) {
            $canonical_array = array($this->uri->segment(1), $page); //Get the news controller name (can be blog, news, etc.) and strap the page on the end
...
//######################################################################################################################
    ...
    public function build_category_keywords() {
        ...
        if (empty ($this->data['page_category'])) {
            $page_category = $this->db->select('category_id')->from('CORE_News_CATEGORY_XREF')->where('news_id', $page_id)->get()->result_array();
            if(!empty($page_category)){
                $category_ids = array();
                foreach($page_category as $cat){
                    $category_ids[] = $cat['category_id'];
                }
                $this->data['page_category'] = $this->db->select('*')->from('CORE_Page_CATEGORY')->where_in('id', $category_ids)->order_by('keyword','asc')->get()->result_array();
                if ($this->cache->configured) {
    ...
    protected function get_data_xref() {
        ...
        if (!empty ($this->template_ids)) {
            ...
            $xref_data = $this->db->get()->result_array();
		}else{
            $xref_data = array();
        }
        if ($this->cache->configured) {
