...
$vardom = array(
    ...
    'COUNT'  => '/^\d+$/',
    // number of line displayed in list
    'SCOPE'  => '/^[AD]$/',
    // list view scope
    'SORT1'  => '/^[AHSMCDTZ]$/',
    // first sort key
...
EOB;
echo
menu_entry(1, 'View Host Stats'), menu_entry(2, 'System Cache Entries');
if ($AUTHENTICATED) {
    echo menu_entry(4, 'Per-Directory Entries');
}
echo
menu_entry(3, 'User Cache Entries'), menu_entry(9, 'Version Check');
...
EOB;
        ...
        if (graphics_avail()) {
            ...
            echo <<<EOB
			<img alt="" $size src="/admin/home/?IMG=3&$time">
EOB;
        }
        echo <<<EOB
            ...
            foreach ($scope_list as $j => $list) {
                foreach ($cache[$list] as $i => $entry) {
                    ...
                    foreach ($entry as $k => $value) {
                        ...
                        echo
                        "<tr class=tr-$m>", "<td class=td-0>", ucwords(preg_replace("/_/", " ", $k)), "</td>", "<td class=td-last>", (preg_match("/time/", $k) && $value != 'None') ? date(DATE_FORMAT, $value) : htmlspecialchars($value, ENT_QUOTES, 'UTF-8'), "</td>", "</tr>";
                        $m = 1 - $m;
                    }
                    if ($fieldkey == 'info') {
...
EOB;
        ...
        if ($list) {
            ...
            foreach ($list as $entry) {
                echo
                '<tr class=tr-', $i % 2, '>', "<td class=td-0>", $entry[0], '</a></td>', '<td class="td-n center">', $entry[1], '</td>', '<td class="td-n center">', $entry[2], '</td>', '<td class="td-n center">', $entry[3], '</td>', '<td class="td-n center">', round($entry[2] / $entry[1]), '</td>', '<td class="td-n center">', round($entry[3] / $entry[1]), '</td>', '</tr>';
